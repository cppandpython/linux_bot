...
def setup():
    ...
    _logname = shell('logname', output=True)
    LOGGED_USER = _logname.strip() if _logname else NULL
    ...
    if not os.path.isfile(_xauth):
        try:
            _luid = getpwnam(LOGGED_USER).pw_uid
        except:
            _luid = '1000'

        for n in os.listdir(f'/run/user/{_luid}'):
            if 'waylandauth' in n:
